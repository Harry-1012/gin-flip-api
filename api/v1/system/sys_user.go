...
import (
	...
	systemReq "github.com/haoleiqin/gin-flip-api/model/system/request"
	systemRes "github.com/haoleiqin/gin-flip-api/model/system/response"
...
// @Success 200 {object} response.Response{data=systemRes.LoginResponse,msg=string} "返回包括用户信息,token,过期时间"
// @Router /base/login [post]
func (b *BaseApi) LoginByApi(c *gin.Context) {
	var l systemReq.Login
	_ = c.ShouldBindJSON(&l)
	if err := utils.Verify(l, utils.LoginVerify); err != nil {
		response.FailWithMessage(err.Error(), c)
	...
	u := &system.SysUser{Username: l.Username, Password: l.Password}
	if err, user := userService.Login(u); err != nil {
		global.GVA_LOG.Error("登陆失败! 用户名不存在或者密钥错误!", zap.Error(err))
		response.FailWithMessage("用户名不存在或者密钥错误", c)
	} else {
		b.tokenNext(c, *user)
	}
}
