<template>
  <div>
    ...
    <el-dialog v-model="dialogFormVisible" :close-on-click-modal="false" :before-close="closeDialog" title="弹窗操作">
    ...
    {{- range .Fields}}
      ...
      {{- else }}
          <el-input-number v-model.number="formData.{{ .FieldJson }}" clearable placeholder="请输入" />
      {{- end }}
      {{- end }}
      ...
      {{- if eq .FieldType "float64" }}
          <el-input-number v-model="formData.{{ .FieldJson }}"  style="width:100%"  clearable />
      {{- end }}
        </el-form-item>
      {{- end }}
      </el-form>
