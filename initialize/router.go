...
import (
	...
	"github.com/haoleiqin/gin-flip-api/router"
)

// 初始化总路由

func Routers() *gin.Engine {
	gin.SetMode(global.GVA_CONFIG.GinModel)
	Router := gin.Default()
	...
	SignApiGroup := Router.Group("")
	// 需签名接口
	SignApiGroup.Use(middleware.CheckSign())
	{
		// xxxRouter.InitXxxRouter(SignApiGroup) // 注册B2C路由
	}
	...
	PrivateGroup.Use(middleware.JWTAuth()).Use(middleware.CasbinHandler())
	{
		...
		systemRouter.InitAppLogRouter(PrivateGroup)              // app请求日志
